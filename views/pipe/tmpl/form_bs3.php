...
//echo '<style href="'.plugin_dir_url('').basename(PIPES_PATH).'/assets/process.css"></style>'.'<br/>';
//echo  '<script href="'.plugin_dir_url('').basename(PIPES_PATH).'/assets/ogb-lib-admin.js"</script>'.'<br/>';

//exit();
/*
$doc	= JFactory::getDocument();
$assets	= 'components/'.$option.'/assets/';
$doc->addStyleSheet($assets.'process.css');
$doc->addScript($assets.'ogb-lib-admin.js', 'text/javascript');
$doc->addScriptDeclaration('window.onload = function() {
							document.getElementById("pipe_title").focus();
							};');
...
");
$doc->addScript('components/com_wppipes/assets/jui/js/chosen.jquery.min.js', 'text/javascript');
...
<script>
	...
	/* It is not very necessary, we can think about it later
	 ...
	 jQuery(document).ready(function(){
	 jQuery('#jform_set_default_obg_<?php echo $item->id;?>').click(function(event){
	 var answer = confirm('<?php echo __('Warning: After you allow this action, all processors you created before will be remove!', true);?>');
	 if(!answer){
	 event.preventDefault();
	 }else{
	 var form = this.form;
	 submitbutton(form,'apply');
	 }
	 });
	 });*/
</script>
...
<!--			'<div id="iwant-link-wrap" class="hide-if-no-js screen-meta-toggle">' +-->
<!--				'<a href="#iwant-wrap" id="show-iwant-link" class="show-settings" aria-controls="iwant-wrap" aria-expanded="false">I want</a>' +-->
<!--			'</div>'-->
<!--		);-->
<!--		var iwant_form = $('#dropdown-iwant').html;-->
<!--		$('#screen-meta').append(-->
<!--			'<div id="iwant-wrap" class="hidden" tabindex="-1" aria-label="I want" style="display:none">' +-->
<!--				iwant_form +-->
<!--			'</div>'-->
<!--		);-->
<!--	})(jQuery);-->
...
<h2>
	...
	if ( ! $item->id ) { // New Pipe
		...
		?>
		<select name="surf" onchange="switch_pipe(document.adminForm.surf.options[document.adminForm.surf.selectedIndex].value);" value="GO" size="1" class="" style="margin-bottom:4px;">
			<option value="1">- switch to other pipe -</option>
			<?php foreach ( $other_pipes as $other ): ?>
				<option value="admin.php?page=pipes.pipe&id=<?php echo $other['id']; ?>"><?php echo $other['id'] . ' - ' . $other['name']; ?></option>
			<?php endforeach; ?>
		</select>
		<?php echo __( '- or -' ); ?>
		<?php
	...
	<div class="alert alert-info alert-dismissable hidden">
		...
		<div class="row">
			<div class="col-md-4">
				...
				<p>Each Pipe is for collecting data/posts from SOURCE >>> process by PROCESSORS (if needed) >>> pass to DESTINATION.<br />
...
<div class="row" style="padding-bottom: 20px;">
	...
	<div class="col-md-6">
		...
		<div class="input-group input-group-lg col-xs-12">
			...
			<input placeholder="<?php echo __( 'Give this Pipe a name, whatever, just a name' ); ?>" onfocus="this.select();" type="text" class="form-control" id="pipe_title" required="true" value="<?php echo $item->name; ?>" id="jform_title" name="jform[name]" />
		...
		<div class="btn-toolbar pull-right" id="toolbar" data-spy="affix1" data-offset-top="60" data-offset-bottom="200">
			...
			<div class="btn-wrapper" id="toolbar-postu">
				<div class="btn-group">
					...
					<ul class="dropdown-menu" role="menu">
						<li>
							<a href="admin.php?page=<?php echo PIPES::$__page_prefix ?>.pipe&task=post&id=<?php echo $item->id; ?>&u=1" class="btn-pipes-post">
...
<div class="row" style="position: relative;">
	...
	<!-- Engine / Source -->
	<div class="col-md-6">
		<img src="<?php echo plugin_dir_url( '' ) . basename( PIPES_PATH ) . '/assets/images/pipe-step-2.png'; ?>" width="145" height="83" />
	...
	<!-- Adapter / Destination -->
	<div class="col-md-6">
		<img src="<?php echo plugin_dir_url( '' ) . basename( PIPES_PATH ) . '/assets/images/pipe-step-3.png'; ?>" height="83" width="145" />
...
<div class="row">
	<div id="fields_matching" class="col-md-12">
		<div class="text-center">
			...
			<a href="javascript:void(0);" onclick="ogb_config_as_default(this);" title="Config as Default" style="display:none;">[ Get Default Config ]</a>
		</div>
			...
			<div class="col-md-7">
				<div class="panel panel-default">
					...
					<div class="panel-footer">
						...
						<p>Click [Click me] button to select an available input field. Processor will generate several output fields.</p>
					</div>
				</div>
			</div>
			<div class="col-md-3">
				<div class="panel panel-warning">
					...
					<div class="panel-footer">
						<p>This area is the input for each item in Destination. [di] is the short-form of the Destination Input.</p>

						<p>Click [Click me] button to select a particular input field from Source Output Fields and Processors Output Fields.</p>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="ob-list-output" id="ogb-list-output">
			<div class="panel panel-info">
				<div class="panel-heading">
					<span class="ogb-close hasTip" title="close this box" onclick="ogb_closeListOp()"><i class="fa fa-times-circle fa-lg"></i></span>
					<strong>
						<span id="change-field" onclick="ogb_update_field('','');">[ <?php echo __( 'Leave blank' ); ?> ]</span>
					</strong>
